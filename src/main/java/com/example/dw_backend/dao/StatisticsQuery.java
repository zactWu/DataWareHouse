...
public class StatisticsQuery implements AutoCloseable{
    ...
    public List<Record> statisticOther(final String Type){
        try(Session session = driver.session()){
            List<Record> ans = session.readTransaction(new TransactionWork<List<Record>>() {
                @Override
                public List<Record> execute(Transaction transaction) {
                    ...
                    switch (Type) {
                        case "actor":
                            result = transaction.run("match (a:Actor) return distinct a.actor as name, a.count as count limit 5;");
                            break;
                        case "director":
                            result = transaction.run("match (d:Director) return distinct d.director as name, d.count as count limit 5;");
                            break;
                        case "label":
                            result = transaction.run("match (l:Label) return distinct l.label as name, l.count as count limit 5;");
                            break;
                        default:
                            return new ArrayList<>();
                    }
